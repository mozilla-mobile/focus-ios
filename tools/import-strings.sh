...
check_script_location() {
  ...
  [[ ! -d Blockzilla.xcodeproj ]] && die "Run this script from ${root_dir} as tools/import-strings.sh"
...
clone_repo() {
  ...
  git clone "${repo}" "${destination}" >> import-strings.log 2>&1 || die "Failed to clone ${repo}"
...
build_localization_tools() {
  ...
  swift build --package-path "${localization_tools_dir}" >> import-strings.log 2>&1 || die "Failed to build LocalizationTools"
...
import_strings() {
  ...
  msg "${YELLOW}[*] Importing strings for ${target_client}...${NOFORMAT}"
