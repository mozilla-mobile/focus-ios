...
#
# For the bigger picture on string import, import see the wiki.
#
#  https://github.com/mozilla-mobile/focus-ios/wiki/Importing-and-importing-Strings
#
...
-t, --target-client     The target client to import strings for. Must be 'focus-ios' or 'firefox-ios'
EOF
  exit
}
...
clone_repo() {
  ...
  git clone "https://github.com/${repo[url]}.git" "${destination}" >> import-strings.log 2>&1 || die "Failed to clone ${repo_name}"
...
build_localization_tools() {
  ...
  swift build --package-path "${root_dir}${localization_tool[dir]}" >> import-strings.log 2>&1 || die "Failed to build LocalizationTools"
...
find_replace_file_text() {
  ...
  q
!
) >> import-strings.log 2>&1 || die "Failed to replace ${file[find]} with ${find[replace]} in ${file_name}"
...
#######################################
# Runs LocalizationTools to import strings to the project.
# Globals:
#   root_dir
#   YELLOW
#   GREEN
...
#######################################
import_strings() {
  declare -n imports lt l10n
  local client
  imports=$1
  lt="${imports[localization_tools]}"
  l10n="${imports[focusios_l10n]}"
  client="${imports[target_client]}"
  clone_repo l10n
  msg "${YELLOW}[*] Importing strings for ${client}...${NOFORMAT}"
  (cd "${root_dir}${lt[dir]}" && swift run LocalizationTools --import \
    --project-path "${root_dir}/${imports[project_dir]}" \
    --l10n-project-path "${root_dir}${l10n[dir]}") >> import-strings.log 2>&1 || die "Failed to import strings for ${client}"
  msg "${GREEN}[+] Strings imported successfully. Moving on...${NOFORMAT}"
}
...
main() {
  ...
  import_strings script_requirements
