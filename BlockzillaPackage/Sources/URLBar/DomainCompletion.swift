...
class CustomCompletionSource: CustomAutocompleteSource {
    ...
    var enabled: Bool {
        return true //Settings.getToggle(.enableCustomDomainAutocomplete)
    }
...
class TopDomainsCompletionSource: AutocompleteSource {
    var enabled: Bool { return true // Settings.getToggle(.enableDomainAutocomplete)
    }
    
    private lazy var topDomains: [String] = {
        let filePath = Bundle.module.path(forResource: "topdomains", ofType: "txt")
